...
class COCOTrain(COCOBase):
    def __init__(
        ...
        transform = A.Compose(
            [A.SmallestMaxSize(max_size=min(resolution)), A.RandomCrop(height=resolution[0], width=resolution[1])],
            additional_targets={"segmentation": "image"},
        )

        super().__init__(
            dataroot, labelroot, stuffthingroot, "train", onehot_segmentation, use_stuffthing, tokenizer, transform
        )
...
class COCOValidation(COCOBase):
    def __init__(
        ...
        transform = A.Compose(
            [A.SmallestMaxSize(max_size=min(resolution)), A.CenterCrop(height=resolution[0], width=resolution[1])],
            additional_targets={"segmentation": "image"},
        )

        super().__init__(
            dataroot, labelroot, stuffthingroot, "val", onehot_segmentation, use_stuffthing, tokenizer, transform
        )
