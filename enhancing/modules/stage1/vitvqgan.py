...
from .layers import ViTDecoder as Decoder
from .layers import ViTEncoder as Encoder
...
class ViTVQ(pl.LightningModule):
    ...
    def validation_step(self, batch: Tuple[Any, Any], batch_idx: int) -> Dict:
        ...
        xrec, qloss = self(x)
        aeloss, log_dict_ae = self.loss(
            qloss, x, xrec, 0, self.global_step, batch_idx, last_layer=self.decoder.get_last_layer(), split="val"
        )
        ...
        if hasattr(self.loss, "discriminator"):
            discloss, log_dict_disc = self.loss(
                qloss, x, xrec, 1, self.global_step, batch_idx, last_layer=self.decoder.get_last_layer(), split="val"
            )
...
class ViTVQGumbel(ViTVQ):
    def __init__(
        ...
        self.temperature_scheduler = initialize_from_config(temperature_scheduler) if temperature_scheduler else None
    ...
    def training_step(self, batch: Tuple[Any, Any], batch_idx: int, optimizer_idx: int = 0) -> torch.FloatTensor:
        ...
        if optimizer_idx == 0:
            self.log(
                "temperature", self.quantizer.temperature, prog_bar=False, logger=True, on_step=True, on_epoch=True
            )
